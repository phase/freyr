package eagle.nest.hub.util.skIP;
...
/**
 * 
 * @author Skyost (http://bukkit.org/threads/class-skip-get-informations-about-your-players.279120/)
 *
 */
public class SkIP {
	...
	public static final SkIPData getIPData(final String ip) {
		try {
			...
			return new SkIPData(json.get(args[0]).toString(), json.get(args[1]).toString(), json.get(args[2])
					.toString(), json.get(args[3]).toString(), json.get(args[4]).toString(), Double.valueOf(json.get(
					args[5]).toString()), Double.valueOf(json.get(args[6]).toString()), json.get(args[7]).toString());
	...
	public static final SkIPDataWeather getWeatherData(final String city, final String countryCode) {
		try {
			final JSONObject weather = (JSONObject) ((JSONArray) ((JSONObject) new JSONParser().parse(httpGet(String
					.format(OPEN_WEATHER_MAP_URL, city, countryCode)))).get("weather")).get(0);
	...
	public enum IPMode {
		...
		IP_API("http://ip-api.com/json/%s", new String[] { "country", "countryCode", "regionName", "city", "zip",
				"lat", "lon", "timezone" }), IP_INFO_DB("http://api.ipinfodb.com/v3/ip-city/?key=" + IP_INFO_DB_API_KEY
				+ "&ip=%s", new String[] { "countryName", "countryCode", "regionName", "cityName", "zipCode",
